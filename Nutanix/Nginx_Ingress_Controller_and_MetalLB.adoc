...
NOTE: Specifically for supporting SUSE CAP, a wild-card DNS record that matches the SVC_FQDN below must be created as part of the CAP deployment process. 
...
NOTE: For a CAP deployment, this test service can be used to hold the first IP address in the range assigned to MetalLB. This would allow for creating a wildcard DNS entry for CAP and this IP address. Then, simply delete the Nginx test deployment and service just before starting the CAP deployment. Be sure to use the spec.loadbalancerIP parameter to reserve the same load balancer IP address for CAP.
