...
### Create and populate a variables file (i.e. cap-pre-installation.vars) on the administrative workstation from which the kubectl and helm commands will be run:
...
TIP: Some implementations of BIND will only allow wildcard records to be added directly to a domain file (often user /var/lib/named/ or /var/lib/named/master). The record would look like `*.scf``````````````````````````IN CNAME``````````````````````````scf.caasp.local.` The named service would need to be restarted after any domain files are edited.
